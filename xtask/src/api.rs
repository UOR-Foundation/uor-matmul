//! The public API lock.
//!
//! The refactor this gate was written for is additive by construction: no
//! public item is removed, renamed, or narrowed. Nothing checked that. The flat
//! re-export lists in `crates/uor-matmul/src/lib.rs` name about ninety items,
...
//! function body or a private module: neither is nameable from outside.
//!
//! # What this locks, and what it does not
//!
//! What is recorded is a *path and a kind*, so what is locked is **presence**:
//! a removal fails, a rename fails as the removal it is, an addition passes.
//!
...
//!
//! A removal or a rename fails; an addition passes and is reported. That
//! asymmetry is the whole point --- the constraint being enforced is "nothing is
//! removed", not "nothing changes", and a gate that failed on additions would be
//! turned off by the first person who added a function. The baseline is rewritten
//! only under an explicit `--write`, exactly as `check-model --write` works, so
//! that accepting a removal is a visible edit to a committed file rather than a
//! green run.
...
# `cargo run -p xtask -- check-api` fails if any line here has no counterpart in
# the source, which is what makes \"no public item is removed or renamed\" a
# checked claim rather than a stated intention. New lines are an addition and
# pass; regenerating is how a deliberate removal is accepted, and the diff is
# then a reviewable edit to this file.
";

/// The public surface has not shrunk: every declaration in the baseline is
...
pub fn check_api(root: &Path, write: bool) -> Result<(), Fail> {
    ...
    if !removed.is_empty() {
        return Err(format!(
            "the public API lost {} declaration(s). The constraint on this work is that no \
             public item is removed, renamed, or narrowed --- every change is additive or a \
             widening --- and a rename is a removal to every caller that named the old \
             spelling. This gate holds the first two of those three words; a narrowing \
             leaves every path in place and is a reviewer's to catch, which the module doc \
             states rather than implies. If a removal is intended, regenerate the baseline \
             with \
             `cargo run -p xtask -- check-api --write` so the loss is a reviewable diff to \
             {}.\n\n{}",
            removed.len(),
            BASELINE_PATH,
            removed
    ...
    let added = current.len() - baseline.len();
    println!(
        "check-api: {} public declarations across {} shipped crates, none removed ({added} \
    ...
    for krate in SHIPPED {
        ...
        for file in &files {
            ...
            for entry in declarations(&text, &public_types) {
                ...
                for seg in module.iter().chain(entry.path.iter()) {
                    path.push_str("::");
                    path.push_str(seg);
                }
...
fn frame_for(statement: &str, public_types: &BTreeSet<String>, depth: i32, closed: bool) -> Frame {
    let shut = |body| Frame {
        depth: depth + 1,
        seg: None,
        scope: Scope::Closed,
