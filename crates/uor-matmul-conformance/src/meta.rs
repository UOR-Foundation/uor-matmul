...
pub fn check_honesty(root: &Path, tests: &BTreeSet<String>) -> std::io::Result<HonestyReport> {
    ...
    let model = match Model::load(&root.join("model")) {
        ...
        Err(e) => {
            report
                .violations
                .push(format!("R10: the model does not load: {e}"));
            return Ok(report);
        }
    };
    ...
    // R9, CM-02: every registered ID has a scenario, and a test named for it.
    for row in &model.ids.id {
        if !scenario_ids.contains(row.id.as_str()) {
            report.violations.push(format!(
                "R9: {} is registered but has no scenario in features/suites/. Every \
    ...
    for s in &suites.scenarios {
        ...
        if s.steps.is_empty() {
            report.violations.push(format!(
                "R9: scenario `{}` in {} has no steps. There are no pending steps.",
                s.statement, s.suite
            ));
        }
        // R4: the scenario's tag must agree with the register.
        if let Some(row) = model.ids.get(&s.id) {
            if s.level != row.level.as_str() {
                report.violations.push(format!(
                    "R4: {} is tagged `{}` in {} but `{}` in the register.",
                    s.id,
                    s.level,
                    s.suite,
                    row.level.as_str()
        ...
        for (i, line) in text.lines().enumerate() {
            ...
            for id in &open_ids {
                ...
                if let Some(word) = ASSERTIVE.iter().find(|w| lower.contains(*w)) {
                    report.violations.push(format!(
                        "R4: {doc}:{}: `{id}` is an `open` claim --- measured and reported, \
    ...
    // R4: a `some-true` authority is reproduced, not established here.
    for authority in &model.authorities.authority {
        for doc in ["README.md", "CONFORMANCE.md"] {
            ...
            for (i, line) in text.lines().enumerate() {
                ...
                if let Some(word) = ASSERTIVE.iter().find(|w| lower.contains(*w)) {
                    report.violations.push(format!(
                        "R4: {doc}:{}: `{}` is cited, not established here, but this line \
                         says `{word}`.\n    {}",
                        i + 1,
                        authority.id,
                        line.trim()
